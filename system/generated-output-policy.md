...
| Graphify compatibility snapshot | `.graphify-out/`, `graphify-out/` (retired 2026-08-07) | Brain execution; Mind interpretation | Retired. These are no longer current output locations. Future operational generated output belongs outside canonical Mind source under the Brain-owned runtime model. | Not applicable; archived compatibility outputs | Ignore; defensive exclusion patterns in `.gitignore` and `.graphifyignore` prevent accidental reintroduction |
| Graphify caches | `runtime/local/graphify/cache/` (current operational); `.graphify-out/cache/`, `graphify-out/cache/` (retired) | Brain | Current: keep operational cache seven days after last use or until invalidated. Retired: never recreate. | Rebuilt by the next authorized `/graphify --update` under operational root | Never track |
| Graph data and semantic reports | `graph.json`, `manifest.json`, `GRAPH_REPORT.md`, `.graphify_labels.json`, `.graphify_root` below `runtime/local/graphify/` (current) or historical compatibility locations (retired) | Brain | Current operational: same as containing runtime snapshot. Historical: never recreate. | Rebuilt by the next authorized `/graphify --update` under operational root only | Never track unless a dated audit explicitly names and justifies a bounded historical copy |
| Visual convenience artifact | `graph.html` below `runtime/local/graphify/` (current) or historical locations (retired) | Brain | Current operational: latest useful render only under operational root. Retired: never recreate. | Rebuilt by the authorized Graphify renderer under operational root when graph size permits | Never track |
